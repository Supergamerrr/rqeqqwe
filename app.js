...
process.on('exit', () => { logger.info('Программа прекращает работу...'); checkForUpdates(); });

(async () => {
	checkForUpdates();
	if (config.proxies.enable_scrapper) {
		sendWebhook(config.webhook.url, `Запуск узла...`);
		logger.info('Скачиваем прокси...');
		...
		logger.info(`Скачано ${chalk.yellow(downloaded.length)} прокси.`);
		sendWebhook(config.webhook.url, `Прокси были загружены на узел.`);
	}
			...
			(err, res, body) => {
				...
				if (body.subscription_plan) {
					...
					if (config.webhook.enabled && config.webhook.notifications.valid_code) {
						sendWebhook(config.webhook.url, `@everyone Найден \`${body.subscription_plan.name}\` подарок в \`${ms(+new Date() - stats.startTime, { long: true })}\` : https://discord.gift/${code}.`);
					}
					...
					if (timeout != 600000) {
						proxies.push(proxy);
						logger.warn(`${chalk.gray(proxy)} достиг лимита. (${(timeout).toFixed(2)}s), ${proxies[0] === proxy ? 'таймаут' : 'Пропускаем прокси...'}...`);
					}
					else {
						logger.warn(`${chalk.gray(proxy)} был заблокирован Discord. Убираем данный прокси...`);
					}
					p = proxies.shift();
				}
	...
	if (config.webhook.enabled && config.webhook.notifications.boot) sendWebhook(config.webhook.url, 'Узел запущен.');
	...
	setInterval(async () => {
		...
		if (stats.threads === 0) {
			...
			if (!proxies[0]) {
				...
				if (config.webhook.enabled) await sendWebhook(config.webhook.url, 'Закончились прокси.');
	...
	setInterval(async () => {
		...
		logger.info('Скачиваем обновлённые прокси.');
